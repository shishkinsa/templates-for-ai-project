...
Документ задаёт **целевой** эталон организации веб-клиента проекта и соглашения по стеку, маршрутизации и работе с API. Нумерация и связь с общими правилами именования сборок сервисов — в [10-app-architecture.md](../10-app-architecture.md). Слои и зависимости backend-сервисов описаны в [11-backend-app-architecture.md](../backend/11-backend-app-architecture.md) (отдельно от клиента). Текущая реализация в репозитории (проект `cnt_sp_web`) может отставать от этой спеки: структура ниже описывает направление развития, а не обязательное совпадение с деревом файлов в каждом коммите.
